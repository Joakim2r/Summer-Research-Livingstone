...
diagnostics <- function(chains, ess, truth_mean = NULL, truth_var = NULL) {
  ...
  compute_esjd <- function(chains) {
    sq_list <- lapply(chains, function(ch) rowSums(diff(ch)^2))
    # this takes the squared difference between rows, then computes the sum across the columns for each row. sq_list should be a n*1 after operations.

    esjd_per_chain <- sapply(sq_list, mean)
    # takes the mean of the squared differences between the rows. This should return a scalar : 1*1

    running_per_chain <- lapply(sq_list, function(v) cumsum(v) / seq_along(v))
    #  returns a list of numeric vectors, Each vector being the running mean for each chain

    running_avg <- Reduce(`+`, running_per_chain) / length(running_per_chain)
    # reduce adds vectors element wise. So you combine the running means for all the chains, and then divide by the number of chains you have
  ...
  mse_first_moment_avg_over_chains <- function(chains, truth_mean) {
    n <- nrow(chains[[1]])
    # gives you the number of iterations

    truth_mat <- matrix(rep(truth_mean, each = n), nrow = n)
    # repeats truth mean on every row

    per_chain <- lapply(chains, function(M) {
      rs <- apply(M, 2, cumsum)
      # gets the running sum for each of the chains

      rm <- sweep(rs, 1, seq_len(n), "/")
      # sweep basically goes row by row, and apply the function '/' which means dividing, and it divides everything by seq_len(n), so each sum is divided by its own length

      rowMeans((rm - truth_mat)^2)
      # length n
    })
  ...
  compute_dt_curve <- function(chains, truth_var) {
    n <- nrow(chains[[1]])
    #Selects the first chain : curve is only computed on the first parameter
  ...
  if (!is.null(truth_mean) && !is.null(truth_var)) {
    ...
    print('No reference Data Available to compute extra Diagnostics !')
  }
